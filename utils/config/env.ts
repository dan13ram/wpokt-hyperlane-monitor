...
export const loadEnvConfig = (): Config => {
  const config: Config = {
    ...
    mongodb: {
      ...
      timeout_ms: getUint64Env('MONGODB_TIMEOUT_MS'),
    },
    ethereum_networks: [],
    cosmos_network: {
      ...
      message_monitor: {
        enabled: getBoolEnv(
          'NEXT_PUBLIC_COSMOS_NETWORK_MESSAGE_MONITOR_ENABLED',
        ),
      ...
      message_signer: {
        enabled: getBoolEnv(
          'NEXT_PUBLIC_COSMOS_NETWORK_MESSAGE_SIGNER_ENABLED',
        ),
        interval_ms: getUint64Env(
      ...
      message_relayer: {
        enabled: getBoolEnv(
          'NEXT_PUBLIC_COSMOS_NETWORK_MESSAGE_RELAYER_ENABLED',
        ),
        interval_ms: getUint64Env(
  ...
  const numEthereumNetworks = getArrayLengthEnv(
    'NEXT_PUBLIC_ETHEREUM_NETWORKS',
  );
